        ...
        <form id="form-assign-menu">
          <div class="form-group">
            ...
            <select id="select_user_group" name="user_group" class="form-control">
              ...
              <?php if(isset($usergroups)){ foreach ($usergroups as $key) { ?>
...
function populate_menu(id){
  $.ajax({
    url: "<?php echo site_url('auth/load_user_unassigned') ?>/" + id,
    type: "POST",
    dataType: "JSON",
...
function assign_menu_to_user(){
  $.ajax({
      url: "<?php echo site_url('auth/assign_menu_to_user') ?>",
      type: "POST",
...
function unassign_menu(id) {
    if (confirm("Are you sure what to un assign menu from user group?")) {
        $.ajax({
            url: "<?php echo base_url('auth/unassign_menu_from_users/'); ?>" + id,
            type: 'POST',
            dataType: 'JSON',
